...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateImages() {
    ...
    updateProfessionName(card2Key); // Pass the middle card's key to update profession name
  }

  function updateProfessionName(card2Key) {
  ...
  setInterval(() => {
    i = (i + 1) % keys.length; // Increment `i` and loop it back when it exceeds the length
    slider();
    updateImages(); // This also calls updateProfessionName
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function slider() {
    // Swap positions visually using `style.left`
    const tempLeft = card1.style.left;
    card1.style.left = card2.style.left;
    card2.style.left = card3.style.left;
    card3.style.left = tempLeft;

    // Swap `zIndex` for depth effect
    const tempIndex = card1.style.zIndex;
    card1.style.zIndex = card2.style.zIndex;
    card2.style.zIndex = card3.style.zIndex;
    card3.style.zIndex = tempIndex;

    // Swap transform scale for sizing
    const tempTransform = card1.style.transform;
    card1.style.transform = card2.style.transform;
    card2.style.transform = card3.style.transform;
    card3.style.transform = tempTransform;

    // Swap box shadows
    const tempBoxShadow = card1.style.boxShadow;
    card1.style.boxShadow = card2.style.boxShadow;
    card2.style.boxShadow = card3.style.boxShadow;
    card3.style.boxShadow = tempBoxShadow;
  }
  ...
  function updateImages() {
    ...
    updateProfessionName(card2Key); // Update the profession name
  ...
  prevBtn.addEventListener("click", () => {
    i = (i - 1 + keys.length) % keys.length; // Decrement `i` and loop back if below 0
    slider();
    setTimeout(() => {
      updateImages();
    }, 700); // Wait for transition before updating content
  });

  nextBtn.addEventListener("click", () => {
    i = (i + 1) % keys.length; // Increment `i` and loop back when exceeding length
    slider();
    setTimeout(() => {
      updateImages();
    }, 700); // Wait for transition before updating content
  ...
  // Auto Slide
  setInterval(() => {
    i = (i + 1) % keys.length; // Increment `i` automatically
    slider();
    setTimeout(() => {
      updateImages();
